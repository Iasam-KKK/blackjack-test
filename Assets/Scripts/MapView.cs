...
namespace Map
{
    public class MapView : MonoBehaviour
    {
        ...
        protected MapConfig GetConfig(string configName)
        {
            ...
            // If not found in list, try to use the one from MapManager
            if (config == null && mapManager != null && mapManager.config != null)
            {
                Debug.LogWarning($"[MapView] MapConfig '{configName}' not found in allMapConfigs list. Using MapManager's config instead.");
        ...
        protected NodeBlueprint GetBlueprint(string blueprintName)
        {
            ...
            if (config == null)
            {
                Debug.LogError($"[MapView] Could not find MapConfig with name '{mapManager.CurrentMap.configName}' in allMapConfigs list. " +
                               $"Make sure to add the MapConfig to the MapView component's 'All Map Configs' list in the inspector.");
                return null;
            }
            
            NodeBlueprint blueprint = config.nodeBlueprints.FirstOrDefault(n => n.name == blueprintName);
            
            if (blueprint == null)
            {
                Debug.LogError($"[MapView] Could not find NodeBlueprint with name '{blueprintName}' in MapConfig '{config.name}'. " +
                               $"Available blueprints: {string.Join(", ", config.nodeBlueprints.Select(b => b.name))}");
            }
